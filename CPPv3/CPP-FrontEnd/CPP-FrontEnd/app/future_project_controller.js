...
angular.module('xenon.future_project_controller', []).
    controller('FutureProjectCtrl', ['MainActivityCategory','currentTrend','TrendId','ProjectTitle','UnitType','SubActivityCategory','Category','GetActivity','$http','$q','$state','$scope', '$compile', 'Program', 'ProgramElement',
        'Project', 'Trend', 'Activity', 'Cost', 'InsertCost','fteposition',
        'FTEPositionCost', 'delayedData', 'Page','UpdateActivity','$filter', 'futureProject','$rootScope','$uibModal', '$timeout',
        function (MainActivityCategory,currentTrend,TrendId,ProjectTitle,UnitType,SubActivityCategory,Category,GetActivity,$http,$q,$state,$scope, $compile, Program, ProgramElement,
            ...
            Page.setTitle('Forecast');
            $scope.allCostTotal = 0;
            console.log("delayedData");
            console.log(delayedData);
            console.log(delayedData[2]);
            ...
            console.log(delayedProject);
            console.log(delayedPhase);
            console.log(delayedActivity);
            ...
            $scope.OrganizationID = delayedData[3];
            ...
            function getProjectValue() {
                ...
                $http.get(serviceBasePath + "Request/TrendGraph/" + delayedProject.ProjectID).then(function (response) {
                    ...
                    //Calculate baseline, current and forecast project duration
                    //var diff = moment(baselineEndDate).diff(moment($scope.currentEndDate), 'days'); //baseline bigger than current
   ...
   /*         $scope.planning_value=  $filter('currency') (phaseArray[0].totalCost,'$',formatCurrency(phaseArray[0].totalCost));
            ...
            console.log('getting $scope.actualBudgetGanttInstance future_project_controller');
            ...
            $scope.scheduleGanttInstance.config.columns = [
                {name: "add", label: "", width: '30px', class:"gantt_add"},
            ...
            $scope.s =   [
                ...
                {name:"mainphase", height:38, map_to:"mainCategory", type:"select",options:
                    ...
                    console.log('calling SubActivityCategory future_ctrl 1');
                        $http.get(serviceBasePath + "Request/SubActivityCategory/" + $scope.OrganizationID + "/"+ index + "/" + $scope.lightBoxTask.parent).then(function(response){
            ...
            $scope.scheduleGanttInstance.config.lightbox.sections = [
                ...
                {name:"mainphase", height:38, map_to:"mainCategory", type:"select",options:
                    ...
                    console.log('calling SubActivityCategory future_ctrl 2');
                        $http.get(serviceBasePath + "Request/SubActivityCategory/" + $scope.OrganizationID + "/"+ index + "/" + $scope.lightBoxTask.parent).then(function(response){
                        console.log(response);
                        var subCategory = response.data.result;
                        for(var i = 0; i < subCategory.length;i++){
            ...
            $scope.scheduleGanttInstance.attachEvent("customSelect",function(id){
                ...
                console.log($scope.selectedActivityCost);
                console.log(MaxFTECostID);
                console.log(MaxLumpSumCostID);
                console.log(MaxUnitCostID);
                var firstChild =null;
                var childTask =null;
            ...
            $scope.$watch('scheduleScale', function () {
                ...
                switch ($scope.scheduleScale) {
                    case "week":
                        var current = futureProject.get({ProjectID:delayedProject.ProjectID, Granularity: 'week'},function(result){
                        ...
                        break;
                    case "month":
                        //var start_date = $scope.scheduleGanttInstance.
                        var current = futureProject.get({ProjectID:delayedProject.ProjectID, Granularity: 'month'},function(result){
                            console.log(result.result);
                            delayedCost = result.result[3];
                            if ($scope.selectedActivity) {
                                if ($scope.selectedActivity.text != "Add") {
                        ...
                        $scope.scheduleGanttInstance.eachTask(function(task){
                            if(task.type !== "project") {
                                //var startDate =  moment(task.original_start_date, "DD-MM-YYYY").startOf('month');
                                //var endDate =  moment(task.original_end_date, "DD-MM-YYYY").endOf('month');
                                var startDate =  moment(task.original_start_date, "DD-MM-YYYY");
                                var endDate =  moment(task.original_end_date, "DD-MM-YYYY");
                                task.start_date = startDate.toDate();
                                task.end_date = endDate.toDate();
                        ...
                        break;
                    case "year":
                        var current = futureProject.get({ProjectID:delayedProject.ProjectID, Granularity: 'year'},function(result){
                            console.log(result.result);
                            delayedCost = result.result[3];
                            if ($scope.selectedActivity) {
                                if ($scope.selectedActivity.text != "Add") {
         ...
         /*   $scope.scheduleGanttInstance.eachTask(function(task){
            ...
            function updateTrendValue(costId){
                ...
                if (1000 === costPhase.id) {
                    $scope.planning_value =  $filter('currency') (phaseTotalCost,'$',(phaseTotalCost % 1 > 0) ? 2 : 0);
                    costPhase["totalCost"] = phaseTotalCost.toString();
                } else if (2000 === costPhase.id) {
                    $scope.schematic_design_value =   $filter('currency') (phaseTotalCost,'$',(phaseTotalCost % 1 > 0) ? 2 : 0);
                    costPhase["totalCost"] =  phaseTotalCost.toString();
                }
                else if (3000 === costPhase.id) {
                    $scope.design_bidding_value =   $filter('currency') (phaseTotalCost,'$',(phaseTotalCost % 1 > 0) ? 2 : 0);
                    costPhase["totalCost"] =  phaseTotalCost.toString();
                }
                else if (4000 === costPhase.id) {
                    $scope.construction_value =   $filter('currency') (phaseTotalCost,'$',(phaseTotalCost % 1 > 0) ? 2 : 0);
                    costPhase["totalCost"] =  phaseTotalCost.toString();
                }
                else if (5000 === costPhase.id) {
                    $scope.closeout_value =  $filter('currency') ( phaseTotalCost,'$',(phaseTotalCost % 1 > 0) ? 2 : 0);
                    costPhase["totalCost"] = phaseTotalCost.toString();
