    ...
    public class FutureTrend
    {
        ...
        public static Trend getBaselineTrend(MySqlConnection conn, MySqlDataReader reader, int ProjectID)
        {
            ...
            try
            {
                ...
                {
                    baselineTrend = ctx.Trend.Where(t => t.TrendNumber == "0" && t.ProjectID == ProjectID).Single();
                }
        ...
        public static List<Trend> getPendingTrends(MySqlConnection conn, MySqlDataReader reader, int ProjectID)
        {
            ...
            try
            {
                ...
                using (var ctx = new CPPDbContext())
                {
                    approvalTrendList = ctx.Trend.Where(t => t.TrendStatusID == 3 && t.TrendDescription != "Baseline" && t.ProjectID == ProjectID).ToList();
                }
                // create and open a connection object
                //conn = ConnectionManager.getConnection();
                //conn.Open();
                ...
                //String query = "SELECT * FROM trend";
                //query += " WHERE 1=1 AND TrendStatusID = 3 And TrendDescription  != 'Baseline' And ProjectID = " + ProjectID;
        ...
        public static Activity getActivityByID(String ActivityID)
        {
            ...
            try
            {
                ...
                using (var ctx = new CPPDbContext())
                {
                    MatchedActivity = ctx.Activity.Where(a => a.ActivityID == aId).Single();
                }
                //conn = ConnectionManager.getConnection();
                //conn.Open();
                ...
                //        MatchedActivity.PhaseCode = reader.GetValue(3).ToString().Trim();
                ...
                //        MatchedActivity.FTECost = reader.GetValue(8).ToString().Trim();
                //        MatchedActivity.LumpsumCost = reader.GetValue(9).ToString().Trim();
                //        MatchedActivity.UnitCost = reader.GetValue(10).ToString().Trim();
                //        MatchedActivity.PercentageBasisCost = reader.GetValue(11).ToString().Trim();
                //    }
                //}
        ...
        public static List<Activity> getActivity(MySqlConnection conn, MySqlDataReader reader, int ProjectID, String TrendNumber)
        {
            ...
            try
            {
                using (var ctx = new CPPDbContext())
                {
                    activityList = ctx.Activity.Where(a => a.TrendNumber == TrendNumber && a.ProjectID == ProjectID).ToList();
                }
                // create and open a connection object
                //conn = ConnectionManager.getConnection();
                //conn.Open();
                ...
                //        activity.ActivityID = (reader.GetValue(0).ToString().Trim());
                //        activity.ProjectID = reader.GetValue(1).ToString().Trim();
                //        activity.TrendNumber = reader.GetValue(2).ToString().Trim();
                //        activity.PhaseCode = reader.GetValue(3).ToString().Trim();
                ...
                //        activity.FTECost = reader.GetValue(8).ToString().Trim();
                //        activity.LumpsumCost = reader.GetValue(9).ToString().Trim();
                //        activity.UnitCost = reader.GetValue(10).ToString().Trim();
                //        activity.PercentageBasisCost = reader.GetValue(11).ToString().Trim();
        ...
        public static List<Cost> getCosts(List<Activity> matchedActivityList, String Granularity)
        {
            ...
            {
                String trendNumber = "";
                List<CostFTE> FTECostList = WebAPI.Models.CostFTE.getCostFTE(ReturnedActivity.ActivityID.ToString(), Granularity, trendNumber,
                                                ReturnedActivity.ProjectID.ToString(),ReturnedActivity.PhaseCode.ToString(),ReturnedActivity.BudgetID.ToString(),ReturnedActivity.BudgetCategory,ReturnedActivity.BudgetSubCategory);
        ...
        public static Activity mergedActivity(Activity baselineActivity, Activity trendActivity)
        {
            ...
            var baselineFTE = (baselineActivity.FTECost != 0.0) ? Convert.ToInt64(baselineActivity.FTECost) : 0.0;
            var trendFTE = (trendActivity.FTECost != 0.0) ? Convert.ToInt64(trendActivity.FTECost) : 0.0;
            mergedActivity.FTECost = baselineFTE + trendFTE;

            var baselineLumpsum = (baselineActivity.LumpsumCost != 0.0) ? Convert.ToInt64(baselineActivity.LumpsumCost) : 0.0;
            var trendLumpsum = (trendActivity.LumpsumCost != 0.0) ? Convert.ToInt64(trendActivity.LumpsumCost) : 0.0;
            mergedActivity.LumpsumCost = baselineLumpsum + trendLumpsum;

            var baselineUnit = (baselineActivity.UnitCost != 0.0) ? Convert.ToInt64(baselineActivity.UnitCost) : 0.0;
            var trendUnit = (trendActivity.UnitCost != 0.0) ? Convert.ToInt64(trendActivity.UnitCost) : 0.0;
        ...
        public static Cost mergeFte(Cost baseCost, Cost actCost)
        {
        ...
        public static Cost mergeUnitCost(Cost baseCost, Cost actCost)
        {
            String[] baseStartDate = baseCost.StartDate.Split(',');
            String[] baseEndDate = baseCost.EndDate.Split(',');
            String[] baseTextBoxVal = baseCost.TextBoxValue.Split(',');
        ...
        public static List<object> mergePendingTrends(List<Trend> ApprovedTrends, int ProjectID, String Granularity)
        {
            Logger.LogDebug(MethodBase.GetCurrentMethod().DeclaringType.ToString(), MethodBase.GetCurrentMethod().Name, "Entry Point", Logger.logLevel.Info);
            Logger.LogDebug(MethodBase.GetCurrentMethod().DeclaringType.ToString(), MethodBase.GetCurrentMethod().Name, "", Logger.logLevel.Debug);
                    ...
                    foreach (var baseCost in baselineCost)
                    {
                        if (actCost.CostType == baseCost.CostType && actCost.Description == baseCost.Description)
                        {
                            if (actCost.CostType == "F")
                            {
                                ...
                                fteCost = mergeFte(baseCost, actCost);
                            ...
                            else if (actCost.CostType == "U")
                            {
                                if (actCost.Base == baseCost.Base && actCost.UnitType == baseCost.UnitType)
                                {
                                    ...
                                    unitCost = mergeUnitCost(baseCost, actCost);
        ...
        public static object GetFutureProject(int ProjectID, String Granularity)
        {
            Logger.LogDebug(MethodBase.GetCurrentMethod().DeclaringType.ToString(), MethodBase.GetCurrentMethod().Name, "Entry Point", Logger.logLevel.Info);
            Logger.LogDebug(MethodBase.GetCurrentMethod().DeclaringType.ToString(), MethodBase.GetCurrentMethod().Name, "", Logger.logLevel.Debug);
            List<object> obj = new List<object>();
            ...
            List<Trend> pendingTrendList = new List<Trend>();   //List containts trend that are waiting for approval
            List<Activity> currentProjectActivityList = new List<Activity>(); //A list that contains all activities of the current project
            List<Activity> pendingActivityList = new List<Activity>();    //A list that contains all activities of the pending trends
            //List<List<Activity>> allApprovedActivityList = new List<List<Activity>>();
            List<Project> project = new List<Project>();       //Store project information
            List<Cost> costList = new List<Cost>();
            List<Cost> currentProjectCostList = new List<Cost>();   //List that contains all the costs of the current-project 
            List<List<Cost>> allCostList = new List<List<Cost>>();  //
            List<Activity> mergedActivityList = new List<Activity>();
            ...
            MySqlConnection conn = null;
            MySqlDataReader reader = null;
            object currentTrendObj = new object();
            currentTrendObj = WebAPI.Models.CurrentTrend.GetCurrentProject(ProjectID, Granularity);
            Trend lastTrend = new Trend();
            IEnumerable<object> collection = (IEnumerable<object>)currentTrendObj;
            project = (List<Project>)collection.ElementAt(0);
            PhaseCodeList = (List<PhaseCode>)collection.ElementAt(1);
            currentProjectActivityList = (List<Activity>)collection.ElementAt(2);
            currentProjectCostList = (List<Cost>)collection.ElementAt(3);


            //var s = currentTrendObj[0];

            //baselineTrend = getBaselineTrend(conn, reader, ProjectID);  //Get baseline trend
            pendingTrendList = getPendingTrends(conn, reader, ProjectID); //get Approval trends as a list
            if (pendingTrendList.Count == 0)
            {
                mergedActivityList = currentProjectActivityList;
                mergedCostList = currentProjectCostList;
            }
            else
            {
                //1. We need to merged all Pending trends together
                //return : list of merged activites and a list of merged costs
                apprTrends = mergePendingTrends(pendingTrendList, ProjectID, Granularity);


                //currentProjectActivityList = getActivity(conn, reader, ProjectID, baselineTrend.TrendNumber);  //Get Baseline Activity
                //currentProjectCostList = getCosts(currentProjectActivityList);  //Get baseline Costs

                List<Activity> mergedPendingActivity = new List<Activity>();  //A list that contains all merged activities of the pending trends
                List<Cost> mergedPendingCost = new List<Cost>();            //A list that contains all merged costs of the pending trends
                mergedPendingActivity = (List<Activity>)apprTrends[0]; //list of merged activities from approved trends
                mergedPendingCost = (List<Cost>)apprTrends[1];     //List of merged Costs from approved trends


                //2. Then we need to merge activities of the approved trends with the baseline activities
                mergedActivityList = combineActivities(mergedPendingActivity, currentProjectActivityList);
                ...
                List<List<Activity>> returnedMatchedActivity = matchedActivities(mergedPendingActivity, currentProjectActivityList);
                ...
                notMatchedActivityList = getUnmatchedActivity(mergedPendingActivity, currentProjectActivityList); //List that stores unmatched activities from both approved trends and baseline

                //4. for activities in baseline that does not have a match, we simply get the costs of that activities
                baselineCost = currentProjectCostList;
                trendActivityCost = mergedPendingCost;
                ...
                foreach (var actCost in returnedCostWithSameActivity)
                {
                    ...
                    foreach (var baseCost in baselineCost)
                    {
                        ...
                        foreach (var act2 in currentProjectActivityList)
                        {
                ...
                for (int i = 0; i < baselineCost.Count; i++)
                {
                    ...
                    foreach (var cost in mergedCostList)
                    {
                        //if(baseCost.ActivityID != )
                        if (cost.CostType == baselineCost[i].CostType && cost.Description == baselineCost[i].Description &&
                            cost.ActivityID == baselineCost[i].ActivityID)
                        {
                            isMatch = true;
                        }
                    }
                ...
                foreach (var cost in costWithoutActivitiyID)
                {
                    Activity activity = getActivityByCost(cost, mergedPendingActivity);
                    if (activity.BudgetCategory != null)
                    {
                        foreach (var act in mergedActivityList)
                        {
            ...
            //{
            //    bool isMatch = false;
            //    foreach (var activity in currentProjectActivityList)
            //    {
            ...
            using (var ctx = new CPPDbContext())
            {
                try
                {
                    List<Trend> currentTrendList = ctx.Trend.Where(a => a.ProjectID == ProjectID).OrderBy(a => a.TrendNumber).ToList();
